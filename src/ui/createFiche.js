...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Bouton PARTAGER UN PROMPT — source unique : config.js
    const btnBetaTest = document.getElementById("btnBetaTest");
    if (btnBetaTest) {
        btnBetaTest.addEventListener("click", () => {
            window.open(BETA_FORM_URL, "_blank");
        });
        console.log("✅ Bouton Partager un prompt initialisé");
    } else {
        console.warn("⚠️ Bouton btnBetaTest introuvable");
    }
...
// ================================================================
// RÉCUPÉRER LES INDICES IA (avec support NC)
// ================================================================
...
async function onGenerate() {
    ...
    if (qrContainer) {
        ...
        try {
            ...
            let qrSize = 512;
            if      (ficheUrl.length > 2700) { qrSize = 1024; }
            else if (ficheUrl.length > 2200) { qrSize = 800;  }
            else if (ficheUrl.length > 1500) { qrSize = 650;  }

            console.log("  - Taille QR retenue:", qrSize, "px");
            ...
            if (ficheUrl.length > 2700) {
                ...
                successMsg.innerHTML = `
                    ...
                    <small style="font-weight:400;">Densité élevée — préférez l'URL cliquable pour partager.</small>
                `;
            } else if (ficheUrl.length > 1800) {
                successMsg.style.color = "#ff9f1c";
                successMsg.innerHTML = `
                    ✅ QR Code généré (${ficheUrl.length} chars)<br>
                    <small style="font-weight:400;">Densité modérée — testez le scan sur mobile si besoin.</small>
                `;
            } else {
                successMsg.style.color = "#1dbf65";
                successMsg.innerHTML = `
                    ✅ QR Code généré avec succès !<br>
                    <small style="font-weight:400;">Scanner ce QR ouvrira directement la fiche.</small>
                `;
            }
            ...
            qrContainer.innerHTML = `
                ...
                <p style='font-size:14px;margin-top:10px;'>
                    La fiche est trop volumineuse pour tenir dans un QR Code.<br>
